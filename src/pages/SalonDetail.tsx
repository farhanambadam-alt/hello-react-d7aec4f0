...
const SalonDetail = () => {
  ...
  const dynamicDistance = salon && userLocation.lat != null && userLocation.lng != null
    ? formatDistance(haversineDistance(userLocation.lat, userLocation.lng, salon.lat, salon.lng))
    : salon?.distance ?? '';
    ...
    >
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-4 mt-2">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[13px] font-body text-muted-foreground">{salon.address} · {dynamicDistance}</span>
              </div>
              <div className="flex items-center gap-1.5">
      ...
      <div className="px-5 pt-5 pb-3 animate-fade-in-up md:hidden" style={{ animationDelay: '80ms', animationFillMode: 'both' }}>
        ...
        <div className="flex gap-2 mt-4 pt-3 border-t border-border items-center">
          <button
            ...
            className="w-11 h-11 min-w-[44px] min-h-[44px] rounded-full btn-themed flex items-center justify-center active:scale-90 transition-transform flex-shrink-0"
      ...
      {activeTab === 'about' && (
        <div className="px-5 pt-4 space-y-3 animate-fade-in-up" style={{ animationDuration: '250ms' }}>
          ...
          {/* ── Hours & Location ── */}
          <div className="bg-card rounded-2xl p-5 card-shadow border border-border">
            <h3 className="font-heading font-semibold text-[15px] text-foreground mb-5">
              Hours & Location
            </h3>
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <p className="text-[14px] font-heading font-semibold text-foreground">
                    {workingHours.find((wh) => wh.day === today)?.time === 'Closed' ? 'Closed Today' : 'Open Now'}
                  </p>
                  <span className={`w-1.5 h-1.5 rounded-full ${
                    workingHours.find((wh) => wh.day === today)?.time === 'Closed' ? 'bg-destructive' : 'bg-success'
                  }`} />
                </div>
                <p className="text-[12px] font-body text-muted-foreground mt-0.5">
                  {today} · {workingHours.find((wh) => wh.day === today)?.time}
                </p>
              </div>
            </div>

            {/* View all hours toggle */}
            <button
              onClick={() => setShowHours(!showHours)}
              className="ml-[52px] mb-4 flex items-center gap-1.5 text-primary active:opacity-70 transition-opacity"
            >
              <span className="text-[12px] font-heading font-semibold">
                {showHours ? 'Hide hours' : 'View all hours'}
              </span>
              <ChevronDown
                size={14}
                className={`transition-transform duration-300 ${showHours ? 'rotate-180' : ''}`}
              />
            </button>
